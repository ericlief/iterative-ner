    ...
    def train(self, 
        ...
        for epoch in range(args.epochs):
            ...
            if train_with_dev:
                ...
                score = self.evaluate(args, dev_data, "dev"+dataset_name[5:], epoch, embeddings_in_memory=embeddings_in_memory)
...
if __name__ == "__main__":
    ...
    import numpy as np
    from convert_wikiner import process_data
    ...
    if args.use_word_emb:
        ...
        word_emb_flair = WordEmbeddings("de")                                                                                       
    ...
    if args.use_l_m:
        ...
        lm = StackedEmbeddings([FlairEmbeddings("de-forward"), FlairEmbeddings("de-backward")])
    ...
    for n in range(5, max_n, 5):
        print(f"Building corpus of size {n}")
        with open("train_PER", "rt") as f:
            data = []
            for i, line in enumerate(f):
                ...
                if line:
                    data.append(line)
        process_data(data, "data/train.conll")        
        ...
        if args.cv:
            ...
            tagger.train(args, corpus.train, "train"+str(n), checkpoint=True, embeddings_in_memory=False, metric="f1")    
            
            # Test 
            score = tagger.evaluate(args, corpus.test, "test"+str(n), test_mode=True, embeddings_in_memory=False, metric="f1")
            scores.append(score) # [(p, r, f1), ...]
         
    
    
    # Save results
    xs = list(range(5, max_n, 5))
    ys = scores
    data = list(zip(xs, ys))
    
    out = logdir + "/" + ",".join(("{}={}".format(re.sub("(.)[^_]*_?", r"\1", key), value) for key, value in sorted(vars(args).items()))) + ".csv"  
    with open(out, "wt") as f:
        for x, (p, r, f1) in data:
            print(f"{x},{p},{r},{f1}", file=f)    
            
            
    # Plot results        
    data = np.loadtxt(out, delimiter=",", dtype=np.float32)
    #print(data.shape)
    plt.plot(data[:,0], data[:,1], color="r", label="prec")
    plt.plot(data[:,0], data[:,2], color="b", label="rec")
    plt.plot(data[:,0], data[:,3], color="g", label="f1")
